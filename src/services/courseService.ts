...
export class CourseService {
  ...
  private async validateTeacher(userId: number): Promise<Teacher> {
    ...
    if (!teacher) {
      throw new BusinessError('FORBIDDEN', ERROR_MESSAGES.BUSINESS.TEACHER_PERMISSION_REQUIRED, 403)
    }
  ...
  private async validateCourseOwnership(courseId: number, teacherId: number): Promise<Course> {
    ...
    if (!course) {
      throw new BusinessError('COURSE_NOT_FOUND', ERROR_MESSAGES.BUSINESS.COURSE_NOT_FOUND, 404)
    }

    if (course.teacher_id !== teacherId) {
      throw new BusinessError('FORBIDDEN', ERROR_MESSAGES.BUSINESS.UNAUTHORIZED_ACCESS, 403)
    }
  ...
  async createCourse(userId: number, courseData: CreateCourseRequest): Promise<CourseBasicInfo> {
    ...
    if (!teacher) {
      throw new BusinessError('TEACHER_NOT_FOUND', ERROR_MESSAGES.BUSINESS.TEACHER_NOT_FOUND)
    }

    if (teacher.application_status !== 'approved') {
      throw new BusinessError('TEACHER_NOT_APPROVED', ERROR_MESSAGES.BUSINESS.TEACHER_NOT_APPROVED)
    }
    ...
    course.survey_url = courseData.survey_url || ''
    course.purchase_message = courseData.purchase_message || ''
    ...
    return {
      ...
      rate: savedCourse.rate || 0,
      review_count: savedCourse.review_count || 0,
      view_count: savedCourse.view_count || 0,
      purchase_count: savedCourse.purchase_count || 0,
      student_count: savedCourse.student_count || 0,
  ...
  async deleteCourse(courseId: number, userId: number): Promise<void> {
    ...
    // 檢查課程狀態（已發布的課程不能直接刪除）
    if (course.status === CourseStatus.PUBLISHED) {
      throw new BusinessError('COURSE_PUBLISHED_CANNOT_DELETE', ERROR_MESSAGES.BUSINESS.COURSE_PUBLISHED_CANNOT_DELETE, 400)
    }
