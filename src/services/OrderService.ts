...
export class OrderService {
   ...
   * 處理訂單付款
   */
  async processPayment(orderId: number, userId: number, paymentData: {
    purchase_way: string
    amount: number
  }): Promise<any> {
    const order = await this.orderRepository.findOne({
    ...
    if (order.status !== OrderStatus.PENDING) {
      if (order.status === OrderStatus.PAID) {
        throw new BusinessError(
          ...
          MESSAGES.BUSINESS.ORDER_ALREADY_PAID,
          400
        )
      } else if (order.status === OrderStatus.CANCELLED) {
        throw new BusinessError(
          ERROR_CODES.ORDER_STATUS_INVALID_FOR_PAYMENT,
          MESSAGES.BUSINESS.ORDER_STATUS_INVALID_FOR_PAYMENT,
          400
        )
      }
    }
    ...
    await this.orderRepository.update(orderId, {
      ...
      purchase_way: paymentData.purchase_way as PurchaseWay,
      paid_at: new Date()
    })
    ...
    return {
      order: updatedOrder,
      payment_info: {
        method: paymentData.purchase_way,
        amount: paymentData.amount,
        status: 'completed'
      },
      payment_url: 'https://sandbox-web-pay.line.me/web/payment/wait?transactionReserveId=mock123',
