...
export const certificateCreateSchema = Joi.object({
  ...
  category_id: Joi.number()
    ...
    .required()
    .messages({
      'number.base': '證書類別必須為數字',
      'number.integer': '證書類別必須為整數',
      'number.positive': '證書類別必須為正數',
...
export const certificateItemSchema = Joi.object({
  ...
  category_id: Joi.number()
    ...
    .required()
    .messages({
      "number.base": "證照分類 ID 必須為數字",
      "number.integer": "證照分類 ID 必須為整數",
      "number.positive": "證照分類 ID 必須為正數",
      "any.required": "證照分類 ID 是必填項目"
    }),
  file_path: Joi.string()
    .trim()
    .max(CERTIFICATE_VALIDATION.FILE_PATH.MAX_LENGTH)
...
/**
 * 前端格式證照 Upsert 項目驗證 Schema
 * 使用與 POST 相同的前端欄位格式，但支援可選的 id 欄位用於更新
 */
export const certificateFrontendUpsertItemSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .optional()
...
}).concat(certificateCreateSchema)

/**
 * 前端格式證照批次 Upsert 驗證 Schema
 * 用於 PUT 請求，使用與 POST 相同的前端欄位格式
 */
export const certificateFrontendUpsertBatchSchema = Joi.object({
  certificates: Joi.array()
    .items(certificateFrontendUpsertItemSchema)
    .min(1)
    .max(20)
    .required()
    .messages({
